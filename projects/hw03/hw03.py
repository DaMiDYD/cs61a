...
def pingpong(n):
    ...
    def get_direction(x):
        ...
        if (x-1) % 8 == 0 or num_eights(x-1):
            ...
            return get_direction(x-1)
    return pingpong(n-1) + get_direction(n)
...
def count_coins(change):
    ...
    """
    "*** YOUR CODE HERE ***"
    def count_largest(change, largest_coin):
        if change == 0:
            return 1
        elif change < 0:
            return 0
            ...
            with_largest = count_largest(change - largest_coin, largest_coin)
            without_largest = count_largest(change, descending_coin(largest_coin))
            return without_largest + with_largest

    return count_largest(change, 25)
