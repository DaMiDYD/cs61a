...
def picky_piggy(score):
    ...
    if score % 6 == 0:
        return 7
    sequence = 142857
    referring_position = 6 - score % 6
    return sequence // (10 ** referring_position) % 10
    # END PROBLEM 2
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who:
            current_strategy, player_score, opponent_score = strategy1, score1, score0
        else:
            current_strategy, player_score, opponent_score = strategy0, score0, score1
        player_score += take_turn(current_strategy(player_score, opponent_score), opponent_score, dice, goal)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who:
            ...
            new_score = score0
        if new_score - last_score > running_high:
            print(new_score - last_score, ' point(s)! That\'s a record gain for Player ', who, '!', sep='')
            return announce_highest(who, new_score, new_score - last_score)
        else:
...
def make_averaged(original_function, trials_count=1000):
    ...
    def average_function(*args):
        total = 0
        current_trails = 0
        while current_trails < trials_count:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    trial = make_averaged(roll_dice, trials_count)
    num_of_dice = 1
    highest_average = 0
    best_choice = 0
    while num_of_dice <= 10:
        result = trial(num_of_dice, dice)
        if result > highest_average:
            highest_average = result
            best_choice = num_of_dice
        num_of_dice += 1
    return best_choice
    # END PROBLEM 9
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if hog_pile(picky_piggy(opponent_score) + score, opponent_score):
        return 0
    else:
