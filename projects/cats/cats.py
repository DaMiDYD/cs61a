...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    screened_paragraphs = [i for i in paragraphs if select(i)]
    if k >= len(screened_paragraphs):
        return ''
    else:
        return screened_paragraphs[k]
    # END PROBLEM 1
...
def about(topic):
    ...
    def checker(line):
        ...
        for word in word_list:
            if any([word == key for key in topic]):
                return True
        return False
    return checker
    # END PROBLEM 2
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    def helper(valid_word):
        return diff_function(typed_word, valid_word, limit)
    if typed_word in valid_words:
        return typed_word
    else:
        all_diff = [diff_function(typed_word, valid_word, limit) for valid_word in valid_words]
        if min(all_diff) > limit:
            return typed_word
        else:
            return min(valid_words, key=helper)
...
def feline_flips(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def helper(start, goal, limit_local):
        if limit_local < 0:
            return limit + 1
        if start or goal:
            if abs(len(start) - len(goal)) > limit_local:
                return limit + 1
            return abs(len(start) - len(goal))
        elif start[0] != goal[0]:
            return 1 + feline_flips(start[1:], goal[1:], limit_local - 1)
        else:
            return feline_flips(start[1:], goal[1:], limit_local)
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        consumed_time_single = []
        for j in range(len(words)):
            consumed_time_single += [times_per_player[i][j+1] - times_per_player[i][j]]
        consumed_time_mul += [consumed_time_single]
    return match(words, consumed_time_mul)
...
def fastest_words(match):
    ...
    for word_index in word_indices:
        ...
        for player_index in player_indices:
            if times[player_index][word_index] < best:
                best = times[player_index][word_index]
                best_player = player_index
        best_players += [best_player]
    best_words = []
    for player_index in player_indices:
        best_word = [words[i] for i in word_indices if best_players[i] == player_index]
